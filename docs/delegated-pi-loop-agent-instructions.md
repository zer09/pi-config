...
Every section marked with `<!-- pi-delegated-instructions:begin:... -->` / `<!-- pi-delegated-instructions:end:... -->` comments below is rendered mechanically from the canonical exports in `instructions.ts`. Routing-derived role lists use documentation placeholders, so routing policy edits do not rewrite this document. Regenerate the sections after any instruction change:
...
`docsync.test.ts` fails when the checked-in content drifts from the canonical instruction exports. The surrounding runtime explanation is manually authored. This marker mechanism manages only these fixed, named sections; it is not a general-purpose Markdown template language.
...
These instructions are injected by `delegate_run` through `promptGuidelines`, generated by `delegateRunPromptGuidelines` in `instructions.ts`. Every flat Pi guideline names its tool. At runtime, the solution and review lines list the roles from the current routing snapshot. The reference section below uses placeholders so policy edits remain isolated to `routing.json`.
...
The file is schema version 2: profiles of ordered model tiers plus an `assignments` object mapping the six role families. Solution and review assignments are ordered profile arrays that derive lettered role ids. Implementation, remediation, verification, and oracle each use one profile string. The oracle self-review model set derives from every tier of the assigned oracle profile; there is no separately duplicated model list.

Validate and inspect the current policy without running the full test suite:
...
The oracle rejects all routing overrides. If the parent already runs any model reachable through the oracle profile, the oracle is skipped before creating artifacts or spawning a process.
