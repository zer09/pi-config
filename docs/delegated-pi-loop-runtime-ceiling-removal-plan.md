...
Supersession note (2026-08-28, diagnostics schema; amended 2026-09-13): new run telemetry uses schema 9 under ADR 0017 (`docs/adr/0017-delegated-schema-8-diagnostic-report-persistence.md`). It keeps ADR 0016's maximum-gap measurement and metadata-only success records, with the exact `success-v9-` prefix and schema-9 completed-sample analyzer eligibility. Failure diagnostics retain a bounded 50 KiB `delegateReport` and 4 KiB watchdog-selected `activeBashCommand`, explicit exceptions to this plan's raw-content exclusions with an accepted private-local secret risk. Historical schema 3-8 files, including `success-v8-` files, remain untouched. The schema-6 body below is historical.
