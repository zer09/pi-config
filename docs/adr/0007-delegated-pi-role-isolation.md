...
- independent review A, B, and C: the same three route maps as solution investigation;
...
- Other Pi sessions can reproduce three-member concurrent pre-implementation solution investigation, parent evidence verification and synthesis, default GLM 5.3/max implementation, fresh three-member concurrent independent review with backup-only fallback maps, OpenAI Codex Sol/high verification, or explicitly selected Z.AI and Claude Code assignments from a compact trigger.
...
- Conditional solution investigation consumes three concurrent provider calls before implementation. Final review consumes three additional fresh calls. A gate remains incomplete when any assigned delegate fails or is unavailable.
...
3. Check configured model IDs for the concurrent A/B/C solution trio with their ordered backup maps, the fresh A/B/C review trio with the same maps, OpenAI Codex Sol/high verification, and Z.AI GLM 5.3/max. Separately record which routes appear in Pi's live available catalog.
...
5. Check global and skill instructions agree on direct bash, bounded child supervision, environment scrubbing, fresh sessions, one mutator, concurrent context-isolated solution investigators, orchestrator evidence verification and synthesis, fresh neutral reviewers, three-member gate semantics, and authorization gates.
