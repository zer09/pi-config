# ADR 0007: Isolate delegated Pi implementation and review roles
...
A successful implementation workflow used the current Pi session as an orchestrator and fresh one-shot Pi processes as implementation, independent-review, finding-verification, and focused-remediation delegates. The useful behavior came from more than merely asking a second model for help:
...
The parent Pi session is the sole orchestrator. Spawned delegates execute their assigned role directly and do not recursively delegate.

Run delegates through direct `bash`, never Context Mode, and omit the spawning tool call's timeout. Use fresh `--print --no-session --approve` processes and run delegates sequentially by default.
...
User instructions and more-specific project workflows may override model selection. Project role templates, finding taxonomies, and release gates take precedence over generic skill skeletons.
...
- Other Pi sessions can reproduce the workflow from a compact automatic trigger.
...
- Model identifiers and thinking levels are maintenance points and must be checked against the current Pi catalog when changed.
...
3. Check global and skill instructions agree on direct bash, no timeout, fresh sessions, one mutator, neutral reviewers, and authorization gates.
4. Check changed Markdown links, placeholders, user-specific paths, secrets, and runtime artifacts.
5. Measure the incremental global-context and skill-catalog cost without requiring paid inference.
6. When CLI/model semantics change materially, run a disposable-fixture delegate smoke only with appropriate authorization.
