...
Accepted. Current-policy note (2026-08-27, renewable liveness): the total-runtime statements in this ADR are superseded by ADR 0013. There is no longer one monotonic 45-minute productive-work deadline and no `timed_out/work_deadline` outcome; total elapsed time never terminates a delegate or gates fallback. Productive work is bounded only by renewable liveness leases (five-minute activity warning, ten-minute activity-idle stop, 30-minute progress warning, renewable 45-minute gap between novel structural checkpoints), stalls carry a fixed `stallCause`, catalog preflight keeps its independent 15-second cap, and failure diagnostics use schema 6. The Decision paragraph beginning "The equal-share route deadline policy is superseded" and every "while work time remains" clause are historical descriptions of the interim policy.
