...
Supersession note (2026-08-29; amended 2026-09-13): ADR 0017 (`docs/adr/0017-delegated-schema-8-diagnostic-report-persistence.md`) governs new schema-9 writes, the exact `success-v9-` prefix, and schema-9 completed-sample analyzer eligibility. Failure diagnostics add bounded exact `delegateReport` and `activeBashCommand` objects with an explicit private-local secret risk. The measurement, 4,096-record success retention, analyzer statistics, and metadata-only success policy remain unchanged. Historical schema 3-8 files, including `success-v8-` records, stay untouched and are not pruning candidates. The schema-7 decision below remains historical evidence.
