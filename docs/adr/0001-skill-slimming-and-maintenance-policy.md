...
The first removal under this policy was the `understand` skill suite. These skills were installed as universal symlinks in `~/.agents/skills` and overlapped with existing Pi tools such as Context Mode and the local graph-first code exploration layer. Removal deleted only the symlinks from the active skill path and left the upstream `~/.understand-anything` checkout untouched.
...
The GitHub CLI pass kept `gh-cli`, but slimmed `SKILL.md` to authenticated GitHub routing, mutation gates, URL normalization, read-only examples, and reference navigation. The command-family library moved to `agent/skills/gh-cli/references/index.md`, while exact generated help remains in per-command reference files.
...
A later old pasted-skills re-setup retired the Chrome DevTools MCP skill group from the active Local Skills because Pi's native browser harness covers the common runtime workflows. The former Chrome update-process doc now serves as reinstall notes only.

The retained-skill pass marked package-provided Context Mode helpers (`context-mode`, `ctx-doctor`, `ctx-insight`, `ctx-purge`, `ctx-stats`, and `ctx-upgrade`) as retained without editing package files because their full bodies load only when triggered and package upgrades may restore `node_modules` content. The same pass retained OpenAI-derived skills (`skill-creator`, `figma`, `figma-create-design-system-rules`, and `figma-implement-design`) and the remaining Matt Pocock skills (`grill-with-docs` and `improve-codebase-architecture`) for optional future workflows rather than removing or slimming them now. A follow-up retained the local `keep it` skills (`context-watcher` and `session-handoff`) because they encode custom routing, safety, and continuity conventions. ADR 0002 later replaced the prior graph-first local skill with `codegraph`, which was then merged into `context-watcher` by ADR 0003.

The old pasted-skills re-setup later retired `context-watcher`, the standalone `codegraph` runtime skill, and `agentmemory` from the active Local Skills. CodeGraph remains available through Pi's native graph-first tooling and global/project guidance, while AgentMemory requires a deliberate future extension restore before use. Another follow-up retained `nlm-skill` and the package-provided `librarian` skill because their NotebookLM automation and source-cited library research workflows are niche enough to keep available when explicitly triggered. The `gh-address-comments` skill was later retired because exact GitHub comment URLs map directly to `gh api`, while PR-wide comment handling caused over-broad routing.
