...
The retained-skill pass marked package-provided Context Mode helpers (`context-mode`, `ctx-doctor`, `ctx-insight`, `ctx-purge`, `ctx-stats`, and `ctx-upgrade`) as retained without editing package files because their full bodies load only when triggered and package upgrades may restore `node_modules` content. The same pass retained OpenAI-derived skills (`skill-creator`, `figma`, `figma-create-design-system-rules`, and `figma-implement-design`) and the remaining Matt Pocock skills (`grill-with-docs` and `improve-codebase-architecture`) for optional future workflows rather than removing or slimming them now. A follow-up retained the local `keep it` skills (`context-watcher` and `session-handoff`) because they encode custom routing, safety, and continuity conventions. ADR 0002 later replaced the prior graph-first local skill with `codegraph`, which is now retained because it encodes the custom graph-first CodeGraph MCP/CLI workflow. Another follow-up retained `nlm-skill` and the package-provided `librarian` skill because their NotebookLM automation and source-cited library research workflows are niche enough to keep available when explicitly triggered. The `gh-address-comments` skill was later retired because exact GitHub comment URLs map directly to `gh api`, while PR-wide comment handling caused over-broad routing.
