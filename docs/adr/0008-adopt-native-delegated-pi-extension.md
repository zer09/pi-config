...
Accepted. The hardcoded route maps, backend values, and pre-tool-only fallback cutoff this ADR preserved were superseded on 2026-08-23 by the extension-owned routing configuration in ADR 0009. On 2026-08-25, ADR 0010 superseded this ADR's inherited extension and skill discovery for delegated children: children now load only the strict extension allowlist and orchestrator-selected skill candidates from the extension-owned resource policy, so the statement that read-only delegates receive the normal extension set no longer holds (they still receive Pi's normal built-in tool set). On 2026-08-27, ADR 0015 superseded the 45-minute wall deadline in the preserved list below: total elapsed time never terminates a delegate, and the 45-minute value is now a renewable maximum gap between novel structural checkpoints. ADR 0009's 2026-09-13 live fallback and persisted-session amendment supersedes fresh-child-only route attempts and the execution `--no-session` flag below. Live reuse remains primary; unusable children resume the same private persisted conversation after positive cleanup, including intentional cross-provider transfer. One recovery allowance resets per route. The ephemeral 0700/0600 run-scoped session is deleted at finalization. Acknowledgement alone does not prove persistence. After positive cleanup, fresh replacement requires compaction-aware durable active-history verification against the exact text-only original built assignment or canonical restart-note form, as specified in ADR 0009. Only valid history without that assignment permits context-free replay on the same selected route, with one non-stacking restart note, cleared acknowledgement, one count increment, and attribution to the prior acknowledged supervised attempt. Invalid or oversized history fails closed before spawn. Unacknowledged live and fresh fallback resend the selected assignment prompt, either the base assignment or the same canonical restart-note form, without another increment or attribution. Interrupted side effects have no exactly-once guarantee. Other supervision, recovery, privacy, and concurrency decisions below stand subject to these amendments.
