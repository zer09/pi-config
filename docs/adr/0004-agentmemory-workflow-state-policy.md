...
Pi already has local workflow mechanisms: plans under `plans/`, session handoffs under `handoffs/`, delegates, global/project routing guidance, and ordinary in-session task planning. Enabling AgentMemory workflow state without a deliberate policy would split task authority across systems and make future agents unsure where live work state belongs.

## Decision

Do not expose AgentMemory workflow/task-state tools in Pi by default.

Pi plans, handoffs, delegates, and normal agent planning remain the default systems for task and workflow state. If AgentMemory is restored, it remains a durable memory and provenance system: recall, preferences, prior decisions, file history, session history, commit provenance, lessons, diagnostics, and explicitly saved non-secret memories.
...
- If restored, the Pi AgentMemory extension remains a curated memory surface, not a second task manager.
...
- If restored, AgentMemory docs and the bundled skill must route workflow state back to Pi plans and handoffs.
