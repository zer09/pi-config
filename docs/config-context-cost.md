...
## 2026-09-13 live and persisted fallback continuation attribution

The new canonical `LIVE_CONTINUATION_PROMPT` is 402 characters and 68 local `o200k_base` tokens, measured offline with `uv run --offline --with tiktoken`. It is sent after a verified same-child route switch with accepted in-memory context, or after positive durable active-history verification for an acknowledged fresh replacement. It is not appended to the initial assignment or to parent tool guidance. Parent startup metadata, schemas, and guidelines have zero delta for this increment. The preserved earlier instruction changes are separate work.

Live reuse remains primary. After positive cleanup of an unusable child, the selected route resumes the same private persisted Pi session. Cross-provider conversation transfer is intentional. History still contributes to later provider input; this local prompt count is not a billed-token estimate or a cache-hit guarantee. The ephemeral run-scoped session uses 0700/0600 containment and is deleted at finalization. Session content, path, and ID never enter diagnostics or model-visible results.

RPC prompt success precedes durable message append in Pi 0.85.1. After positive cleanup, fresh replacement verifies an exact text-only user assignment in compaction-aware active context. Only the latest compaction on the root-to-leaf active path applies: a valid earlier legacy boundary or exact `retainedTail` messages plus post-compaction entries determine available assignment context. Usable history reuses the unchanged 68-token continuation without restart attribution. Only valid history without the assignment activates context-free replay on the same selected route. This fail-safe pays for the full original built assignment plus one canonical restart note, clears acknowledgement, increments the existing restart count once, and marks the prior acknowledged supervised attempt. The prompt is rebuilt from the original, so notes never stack. Every unaccepted live or fresh fallback resends its selected assignment prompt: the base assignment before degradation or the same canonical restart prompt after degradation. Rejection adds no restart increment or attribution. Invalid or oversized history fails closed before another provider prompt; the private read has fixed 64 MiB session, 4 MiB line, and 100,000 physical-record bounds. History inspection adds no model-visible text. No instruction text, schema, or parent startup text changed in this remediation, so no recount is required. Continuity cannot guarantee exactly-once interrupted side effects. Provider calibration remains unperformed; no paid inference or full startup inventory ran.
...
## 2026-08-20 delegated Tabitoken route-policy attribution

This change was measured locally with `tiktoken` `o200k_base`; no paid provider calibration or full extension/tool inventory rerun was performed. Before values are the `HEAD` revision of each file.

| Surface | Before | After | Delta | Startup behavior |
|---|---:|---:|---:|---|
| Raw `agent/AGENTS.md` | 2,582 | 2,582 | 0 | Always loaded through the context-file block |
| `delegated-pi-loop` description | 83 | 87 | +4 | Loaded in the skill catalog |
| `delegated-pi-loop/SKILL.md` | 3,383 | 3,558 | +175 | Loaded only when the skill is read |
...
The child prompt now presents the assignment before attempt and terminal details. Its attempt rule remains semantic (two materially equivalent attempts, repeat only with new evidence) and contains no wall-clock instruction; at ADR 0012 the supervisor still owned the programmatic five-minute warning, ten-minute idle termination, and 45-minute productive-work deadline. (Historical: ADR 0015 later removed that 45-minute productive-work deadline and replaced it with renewable programmatic liveness leases; the token counts in this section were measured against the ADR-0012-era wording and are intentionally not recalculated.) The terminal reason lists are generated from the same closed enums used by the parser. Recovery still sends one programmatic `prompt-2` message to the same child session, but refers to the original final protocol instead of repeating every reason code.
