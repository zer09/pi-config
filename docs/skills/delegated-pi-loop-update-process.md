...
- Source of truth: this Pi config, ADR 0007 through ADR 0015, and installed Pi RPC/extension documentation.
...
| `agent/extensions/delegated-pi-loop/diagnostics.ts` | Bounded private schema-7 run telemetry: failure diagnostics plus best-effort metadata-only success records with 4,096-record retention. |
| `agent/extensions/delegated-pi-loop/analyze-progress-gaps.ts` | Local read-only `analyze:progress-gaps` percentile analyzer over schema-7 completed-run samples; aggregate-only output, no writes, no network. |
...
2. Read ADR 0007 through ADR 0015, this document, and every owned source file.
