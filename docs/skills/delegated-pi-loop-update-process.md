...
- Gate A runs Muse Spark then AgentRouter Sol; Gate B runs DeepSeek then AgentRouter Opus; Gate C runs `tokenreply/ox-alpha:xhigh`, Hy3, then AgentRouter Opus. Tabitoken and GoRouter have no capability or tier entries in delegated routing. Every A/B/C tier allowlists exactly one provider and is deterministic without a random draw. Gate D runs `gpt-5.5` at thinking `high`; Solution E and the Oracle each run `gpt-5.6-sol` at thinking `high` on the nine eligible OpenAI Codex providers `openai-codex`, `openai-codex-zahlo`, and `openai-codex-cgpt1` through `openai-codex-cgpt7`; Cursor stays excluded. Inside those three multi-provider chains the primary is the eligible parent provider, otherwise exactly one random draw, and the remaining providers follow in stable config order.
...
1. Solution A/B/C/D/E and review A/B/C/D roles retain their concurrent gates; a temporary extra reviewer reuses an existing non-exclusive review role under the same overlap rules, and the manager admits duplicate review roles by design.
