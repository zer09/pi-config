...
| `agent/extensions/delegated-pi-loop/diagnostics.ts` | Bounded private schema-8 run telemetry: failure diagnostics that retain the exact final report bounded to 50 KiB with recognized terminal lines preserved, plus best-effort metadata-only success records with 4,096-record retention. |
| `agent/extensions/delegated-pi-loop/analyze-progress-gaps.ts` | Local read-only `analyze:progress-gaps` percentile analyzer over schema-8 completed-run samples; aggregate-only output, no writes, no network. |
...
12. The parser accepts only exact fixed codes. Unknown, malformed, duplicate, misplaced, path-like, credential-like, overlong, Unicode, or outcome-mismatched values are discarded; a BLOCKED/FAILED outcome then stays terminal with reason `unspecified` plus status `rejected`, and a bare legacy marker stays terminal with `unspecified` plus `missing`. Delegate-authored free text is never rendered or persisted as a parsed terminal reason; the bounded private failure-only `delegateReport` (ADR 0017) is the sole persistence exception.
...
5. Never retain raw provider errors, billing text, balances, account data, credentials, prompts, reports, or protocol payloads in status, rendering, or failure Markdown. The failure diagnostic's bounded 50 KiB `delegateReport` object (ADR 0017) is the single exception: it persists the exact final report of unsuccessful runs only, and no other surface, status, or telemetry ever carries report text.
...
8. Run telemetry uses schema version 8 and includes only bounded fixed causes (`stallCause` plus the remaining catalog, cleanup, interruption, and terminal-reason categories), the idle ages including `maxProgressIdleSeconds` (the attempt's maximum structural-progress gap: attempt start to first novel checkpoint, checkpoint to checkpoint, and last checkpoint to settlement; zero survives, negative and non-finite values are omitted), activity, structural-progress, duplicate-checkpoint, and warning counts, state, delegate outcome, terminal reason and status, route, timing, recovery metadata, provider category, active-tool metadata, sanitized progress, attempts, and stream error categories. Unsuccessful runs write the failure diagnostic with its existing writer and ToolResult/TUI path exposure, plus one `delegateReport` object: the exact final report bounded to 50 KiB, byte-for-byte within the limit, and with a recognized terminal `DELEGATE_REASON`/`DELEGATE_RESULT` suffix preserved verbatim at the end over a UTF-8-safe body prefix when oversized; `totalBytes` keeps the original UTF-8 size and `truncatedBytes` the omitted bytes. The report object never reaches the parent ToolResult, rendering, or any model-visible surface. Completed runs also write one best-effort metadata-only success record with the exact `success-v8-` filename prefix, never model-visible and never in ToolResult content or details, with create, write, chmod, and prune failures never changing the outcome. Success retention keeps the newest 4,096 exact `success-v8-*.json` regular files through no-follow `lstat` checks with deterministic write-time plus filename ordering, `ENOENT` tolerance, and in-process write-plus-prune serialization; failures, historical files, unknown names, directories, and symlinks are never pruned. Existing schema 3 through 7 files are not migrated. Outside the verbatim failure-only report object, no delegate-authored reason text, checkpoint digest, or HMAC key is included.
