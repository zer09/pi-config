...
8. Failure diagnostics use schema version 6 and include only bounded fixed causes (`stallCause` plus the remaining catalog, cleanup, interruption, and terminal-reason categories), the three idle ages, activity, structural-progress, duplicate-checkpoint, and warning counts, state, delegate outcome, terminal reason and status, route, timing, recovery metadata, provider category, active-tool metadata, sanitized progress, attempts, and stream error categories. Existing schema 3, 4, and 5 files are not migrated. No delegate-authored reason text, checkpoint digest, or HMAC key is included.
