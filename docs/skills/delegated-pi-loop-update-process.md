...
Run strict TypeScript with `strict`, `noUnusedLocals`, and `noUnusedParameters` against the installed Pi declarations. The maintained check is a temporary config: create a transient `tsconfig` in the extension directory with `strict`, `noUnusedLocals`, `noUnusedParameters`, `noEmit`, `skipLibCheck`, `target: es2023`, `module: esnext`, `moduleResolution: bundler`, `allowImportingTsExtensions`, `verbatimModuleSyntax`, `erasableSyntaxOnly`, `noUncheckedSideEffectImports`, `types: ["node"]` with `typeRoots` on the installed `@types`, `paths` mapping `@earendil-works/pi-coding-agent`, `@earendil-works/pi-tui`, `@earendil-works/pi-ai`, and `typebox` to the installed declaration files under `~/.bun/install/global/node_modules`, and `include: ["*.ts"]`, then run `node ~/.bun/install/global/node_modules/typescript/bin/tsc -p <config>` and delete the transient config before committing. It is not a maintained project artifact.
