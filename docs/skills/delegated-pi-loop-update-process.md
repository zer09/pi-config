...
7. The review gate stays strict: every configured reviewer must complete before the gate passes. A non-completed reviewer blocks the gate unless an applicable exact `OVERRIDE:` directive names the failed role and current gate. Record the override and never label a failed reviewer as passed. Without a broader override, findings from completed reviews remain binding and follow normal verification and remediation. Gate completion remains parent-side orchestration policy; the extension enforces no override state.
8. The solution-investigation gate stays strict in parallel: every configured investigator must complete before synthesis. A non-completed investigator blocks the gate unless an applicable exact `OVERRIDE:` directive names the failed role and current gate. Record the override and never label a failed role as completed or passed. Without a broader override, synthesis still requires at least one completed report and uses only completed reports plus parent-verified repository evidence. Gate completion remains parent-side orchestration policy; the extension enforces no override state.
