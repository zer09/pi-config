...
const bundledDetectCommand = `${JSON.stringify(process.execPath)} ${JSON.stringify(
  path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../detect.mjs'),
)}`;
            ...
            if (fwConfig) {
              ...
              if (probe.listening && probe.matched) {
                process.stderr.write(
                  ...
                  `  ${bundledDetectCommand} http://localhost:${fwConfig.port}\n\n`
                );
