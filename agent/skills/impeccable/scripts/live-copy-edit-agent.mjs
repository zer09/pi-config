#!/usr/bin/env node
// Locally adapted for Pi secret-placeholder safety; see docs/skills/impeccable-update-process.md.
/**
...
export function describeNoProviderError({
  ...
  if (exists('claude')) {
    if (env.CLAUDE_CODE_OAUTH_TOKEN) {
      ...
      lines.push('      Headless fix: run `claude setup-token` once, then `export CLAUDE_CODE_OAUTH_TOKEN=<token printed by claude setup-token>` before starting `live-server.mjs`.');
