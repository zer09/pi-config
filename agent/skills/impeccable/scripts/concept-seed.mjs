#!/usr/bin/env node
// Locally adapted for Pi safety; see docs/skills/impeccable-update-process.md.
/**
 ...
 * never leave the machine. This local Pi copy disables the ping by default.
 * The exact command must set IMPECCABLE_ALLOW_TELEMETRY=1 after explicit user
 * authorization; DO_NOT_TRACK or IMPECCABLE_NO_TELEMETRY still disables it.
 *
 * Env vars:
 ...
 *   IMPECCABLE_ALLOW_TELEMETRY — enables one authorized choice ping.
...
function telemetryDisabled() {
  // Local Pi policy keeps hosted-service writes off unless the user approves
  // this exact choice ping and the caller opts in for that command.
  return process.env.IMPECCABLE_ALLOW_TELEMETRY !== '1'
    || Boolean(process.env.IMPECCABLE_NO_TELEMETRY || process.env.DO_NOT_TRACK);
}
...
rivals to your habitual layout, and keep only what makes this product clearer.${grainNote}\n`
  ...
  // (anonymous telemetry on API-dealt rolls; locally disabled unless the
  // exact command sets IMPECCABLE_ALLOW_TELEMETRY=1) and opens the build's phase machine, whose
  ...
  It records the choice${data.source === 'api' ? ' locally; the anonymous remote ping runs only when the exact command sets IMPECCABLE_ALLOW_TELEMETRY=1 after user authorization' : ''} and opens the build phases: on a comp-led build the comp round is the first gate (three comps, one approved) and no page code is written before it closes; on a code-led build it prints the contract step. A build without this state file is a build the finish reviewer treats as having skipped the round.\n`
    : (data.source === 'api'
