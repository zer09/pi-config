// Locally adapted for Pi update safety; see docs/skills/impeccable-update-process.md.
/**
...
// can offer the local reviewed update process. Everything here is best-effort and
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    ...
    `Update it through the reviewed process named by the installed skill's Maintenance section." ` +
    `Never run \`npx impeccable update\` against this locally adapted copy: it would replace the safety, path, and Pi compatibility overlays. ` +
