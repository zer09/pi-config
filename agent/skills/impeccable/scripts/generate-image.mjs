#!/usr/bin/env node
// Locally adapted for Pi hosted-service safety; see docs/skills/impeccable-update-process.md.
/**
 ...
 * user's API credit (roughly $0.05-0.25 per image at default quality). Local
 * Pi policy requires explicit user authorization for the exact call and the
 * per-command IMPECCABLE_ALLOW_IMAGE_API=1 opt-in. Reference images are
 * uploaded to OpenAI by the edits endpoint and must be disclosed before use.
 *
...
if (process.env.IMPECCABLE_ALLOW_IMAGE_API !== '1') {
  console.error('generate-image: remote image generation is disabled. After explicit user authorization for this billed call and any --ref uploads, rerun this exact command with IMPECCABLE_ALLOW_IMAGE_API=1. Use IMPECCABLE_IMAGE_GEN_FAKE=1 for offline validation.');
  process.exit(1);
}
