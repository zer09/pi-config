...
1. `live.mjs`: boot. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/live.mjs" --target <path>` instead; then run the rest of this live session from the returned `projectRoot`. The boot resolves the app root from dev-server config files and persists it in `.impeccable/live/roots.json`; every helper re-anchors to that manifest at startup (a wrong cwd cannot fork session state), PRODUCT.md / DESIGN.md are discovered upward to the git root, and relative helper args like `--file` resolve against the app root.
...
```
LOOP:
  node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/live-poll.mjs"   # default long timeout; no --timeout=
...
```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/live.mjs"
```
...
```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/live-status.mjs"      # helper state, active sessions, queued events; works with the helper down
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/live-resume.mjs" --id SESSION_ID   # active snapshot, pending event, next safe action
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/live-complete.mjs" --id SESSION_ID # canonical manual final acknowledgement after verified cleanup
```
...
```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/live-insert.mjs" --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/live-wrap.mjs" --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/live-poll.mjs" --reply EVENT_ID done --file RELATIVE_PATH
```
...
On Pi, load [degraded/manual-edit-applier.md](degraded/manual-edit-applier.md) and apply the source edits inline; do not map this role to `delegate_run`. On another harness with compatible native subagents, delegate source edits to `impeccable_manual_edit_applier` / `impeccable-manual-edit-applier`. Pass cwd, scripts path, event id, page URL, chunk/deadline, `batch`, `evidencePath`, and the canonical JSON result schema. The subagent must not poll or reply. If a compatible native agent is unavailable, apply inline with the degraded contract.
...
```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/live-server.mjs" stop
```
