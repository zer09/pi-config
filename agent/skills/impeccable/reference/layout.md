...
On Pi, run these sequentially inline; `delegate_run` does not provide these Impeccable assessment roles. On another harness with compatible native sub-agents, run them independently; otherwise run them yourself in this order.
...
```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/detect.mjs" --json --scope layout [target files or dirs]
```
