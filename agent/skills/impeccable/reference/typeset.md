...
On Pi, run these sequentially inline; `delegate_run` does not provide these Impeccable assessment roles. On another harness with compatible native sub-agents, run them independently; otherwise run them yourself in this order. Do not let detector findings anchor the design assessment.
...
```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/detect.mjs" --json --scope type [target files or dirs]
```
