...
This harness has no compatible Impeccable native-subagent capability, so you are running this role inline. A generic fixed-role delegation tool does not provide this role. Step fully out of the work you just finished, adopt only this file's instructions for the pass, and disclose the substitution in one line when you report. Where the text below addresses a parent agent, you are both parties: produce the full output contract first, then act on it yourself.
...
1. `node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/comp-spec.mjs" --crop <id>` writes the reference crop under `.impeccable/build/crops/`.
2. Produce the plate. With the API fallback: `node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/generate-image.mjs" --plate <id> --quality high` does the whole step (crop as reference, the spec's plate prompt, output size chosen from the region's aspect, the file written to its plate path, prompt embedded, and the plate scored against the crop). With a harness-native image tool: use the crop as the input image and `node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/comp-spec.mjs" --plate-prompt <id>` as the prompt, write the result to the plate path, then run `node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/embed-prompt.mjs" <plate> --prompt "<the exact prompt>"`.
