<!-- Locally adapted for global Pi commands and policy; see docs/skills/impeccable-update-process.md. -->

# /skill:impeccable hooks

Manage the **design detector hook** for the current project.

**Pi limitation:** Pi has no compatible Impeccable hook surface. For a normal Pi project, do not run `hook-admin.mjs`, do not install another harness's manifest, and do not claim that automatic detection is enabled. Offer the bundled manual detector instead. Continue with the provider-specific material below only when the user explicitly asks to manage that exact non-Pi harness in the current project.
...
Manual `node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/detect.mjs"` scans use the same project filter config by default: `detector.ignoreRules`, `detector.ignoreFiles`, `detector.ignoreValues`, and `detector.designSystem.enabled`. `hook.enabled` only controls automatic hook execution, not manual scans. Add `--no-config` for a raw detector run that ignores project config/context. Use `hook-admin.mjs` only for explicitly requested ignore-config changes.

Supported harnesses: Claude Code (`.claude/settings.local.json` in the project, which is gitignored so the hook stays machine-local; a hook you move into the shared `settings.json` is honored in place too), Codex (`.codex/hooks.json` in the project), Cursor (`.cursor/hooks.json` in the project), Grok Build (`.grok/hooks/impeccable.json` in the project; requires `/hooks-trust` or `--trust`), and GitHub Copilot (`.github/hooks/impeccable.json` in the project, a team-shared committed file that both the Copilot CLI and the cloud agent read). Pi is not supported. For the Copilot CLI, repo-level hooks fire once `.github/hooks/impeccable.json` is committed to the repository's default branch.
...
1. On Pi, report that automatic Impeccable hooks are unavailable.
2. Offer this manual scan at the workflow's detector checkpoint:

   ```bash
   node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/detect.mjs" --json [target]
   ```

3. If the user explicitly named a supported non-Pi harness and requested a hook mutation, resolve the action and run:

   ```bash
   node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/hook-admin.mjs" <action> [args...]
   ```

4. Pass script output through verbatim. The default ignore scope is shared `.impeccable/config.json`; add `--local` only when the user explicitly asks for a private exception.
...
- If the finding has no value-specific command, such as `side-tab`, scope that one rule to the file: `ignore-value <id> "*" --file <path>`. Run `node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/detect.mjs" <path>` first to see what actually fires there.
...
```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/hook-admin.mjs" ignore-value overused-font Inter --shared --reason "User confirmed Inter is intentional"
```

Example self-served exception, with the evidence named:

```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/hook-admin.mjs" ignore-value bounce-easing bounce-ball --shared --reason "Agent: literal ball-bounce animation, bounce easing is the subject"
```

Example whole-rule font exception:

```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/hook-admin.mjs" ignore-rule overused-font --all-values --reason "User asked to ignore overused fonts generally"
```
...
```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/hook-admin.mjs" ignore-value design-system-font-size "*" --file "src/overlay/widget.js" --reason "Injected widget builds its own type scale; DESIGN.md's ramp describes the site"
```

Example whole-file exception, for a file that is out of scope entirely:

```bash
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/hook-admin.mjs" ignore-file "src/legacy/Card.tsx"
```
...
- On Pi, automatic hooks are already unavailable. Do not write hook config just to represent that limitation. For a supported non-Pi harness, `/skill:impeccable hooks off` writes `hook.enabled: false` for the project; `IMPECCABLE_HOOK_DISABLED=1` remains a one-shell override.
