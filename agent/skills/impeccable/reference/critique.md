...
- Assessment A and B MUST run as two isolated sub-agents whenever the harness exposes Impeccable-compatible native sub-agents. Pi does not: `delegate_run` has fixed repository roles and Impeccable is excluded from delegated children. On Pi, run the assessments sequentially inline and report the required degraded banner.
...
2. **Confirm the target slugs cleanly**:
   ```bash
   node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/critique-storage.mjs" slug "<resolved-path-or-url>"
   ```
...
Use separate Impeccable-compatible native sub-agents for Assessment A and Assessment B when the harness provides them. They must not see each other's output. Do not show findings to the user until synthesis.

Sub-agent gate (all harnesses):
- Pi always takes the inline fallback for these assessments. Do not map them to `delegate_run`, even though that tool is exposed.
- On other harnesses, spawn A and B as two isolated, parallel sub-agents when an Impeccable-compatible native sub-agent/Task tool is exposed.
- "Unavailable" means no compatible Impeccable sub-agent path exists in this session, or the user declined on a harness that asks. A generic fixed-role delegation tool does not make these roles available.
- When compatible sub-agents are unavailable, fall back sequentially: finish and record Assessment A, then run Assessment B, then synthesize, and emit the degraded banner.
- Whichever path you take, declare it in the report header (see Report header provenance). Skipping compatible sub-agents without the banner is a failed critique.
...
4. If mutation is available, start `node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/live-server.mjs" --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
- Degraded: `⚠️ DEGRADED: single-context (<reason, e.g. no compatible Impeccable native sub-agent>)`
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"max_score":<n>,"na_heuristics":"<comma-separated numbers, or empty>","p0_count":<n>,"p1_count":<n>}' \
     node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/critique-storage.mjs" write "<resolved target>" <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/critique-storage.mjs" trend "<resolved target>" 5
   ```
