...
- **Tool version.** The installed skill is older than the published one. `context.mjs` reports that at boot as `UPDATE_AVAILABLE`. Do not run the upstream updater against this locally adapted copy; tell the user to follow `docs/skills/impeccable-update-process.md` in the Pi config repository. Not this command's job.
...
```
node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/doctor.mjs" --json
```
...
- **`auto`** carries no decision. Run `node "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/impeccable/scripts/doctor.mjs" --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
