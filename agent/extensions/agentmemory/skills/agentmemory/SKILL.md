...
- When gated tools are registered, destructive or high-risk wrappers still require exact local `confirm` phrases such as `save agentmemory lesson`, `consolidate agentmemory`, `reflect agentmemory`, `list agentmemory insights`, `audit agentmemory`, `export agentmemory`, `heal agentmemory`, `delete memories:<sorted-ids>`, or `<operation> slot:<label>`. Do not invent confirmations; follow the tool schema.
