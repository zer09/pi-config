...
- Query tools run a full reconciliation at most every 10s as a watcher-independent safety net. When the watcher reports pending files, a query waits up to 10s for its debounce/sync pipeline (including mid-sync follow-up events). If that wait times out, one shared direct reconciliation proceeds without clearing the watcher-owned queue; concurrent queries await the same full reconciliation and post-sync drain.
- Up to four recently used roots are watched concurrently; a healthy least-recently-used watcher is evicted only after an incoming watcher starts successfully. Older roots remain cached and are caught up before their next query.
- An unavailable or degraded watcher retries after a 10s backoff. Degradation invalidates query freshness immediately, forcing direct reconciliation on the next tool call without per-call watcher restart hammering.
