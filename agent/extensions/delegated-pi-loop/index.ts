...
const DelegateParameters = Type.Object({
  role: StringEnum(DELEGATE_ROLES, {
    description: "Assigned isolated role. Use solution A/B/C/D/E and review A/B/C/D concurrently for their required gates.",
  }),
  prompt: Type.String({
    minLength: 1,
...
export default function delegatedPiLoopExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool<DelegateToolParams>({
    ...
    promptGuidelines: [
      ...
      "When no accepted solution contract exists and the root cause, architecture, or approach requires investigation, call delegate_run for solution-a, solution-b, solution-c, solution-d, and solution-e concurrently with the same neutral assignment; all five must complete before synthesis. Solution delegates may gather evidence and propose options, but the parent verifies the evidence, synthesizes conclusions, and remains sole author and owner of the final plan or research deliverable.",
