...
function roleContract(role: ResolvedRole): string {
  // Family-owned contracts: the switch is exhaustive over the six supported
  // families and an unrecognized family throws instead of falling through to
...
Report changed paths, implementation summary, exact checks and results, and remaining risks.`;
    ...
    default: {
      const unknown: never = role.family;
      throw new Error(`no role contract for family "${String(unknown)}"`);
    }
  }
}
