...
export function delegateRunPromptGuidelines(
  ...
  return [
    ...
    "delegate_run [Fast path]: For a simple task with an obvious established pattern, the parent finalizes the scope and executes the task directly without a delegate. For a non-trivial task with an accepted solution contract, skip solution and oracle and run exactly one implementation delegate.",
    ...
    "delegate_run [Gate failure]: Any required non-completed solution or review role leaves automatic gate advancement incomplete. Report the failed role(s), preserve completed evidence, and follow the user's next instruction. Never label failures completed or passed.",
    "delegate_run [Partial evidence]: If the user directs continuation from a partial solution gate, require at least one completed report and synthesize only from completed reports plus parent-verified repository evidence. Findings from completed reviews remain binding unless the user explicitly directs otherwise.",
    ...
    "delegate_run [Failure and authority]: Treat every non-completed state as a failed tool-error delegation and report it. Do not retry automatically beyond bounded fallback. Follow the user's ordinary next instruction; continue, resume, or retry requires no special syntax. Delegate completion never authorizes staging, committing, pushing, deploying, or hosted-service mutation; each requires separate explicit authorization.",
