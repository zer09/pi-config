...
test("eligible records require schema 9, completed invocation, and a completed attempt with a valid maximum", () => {
  ...
  // Historical schema 3-8 and unknown versions.
  ...
  for (const schemaVersion of [1, 2, 10, 99]) {
    assert.deepEqual(
      eligibleGap({ ...completedRecord(1), schemaVersion }),
  ...
  assert.deepEqual(eligibleGap({ ...completedRecord(1), schemaVersion: "9" }), { status: "ignored", reason: "unknownSchemaVersion" });
...
test("analyzeRecords counts every ignored category separately", () => {
  const analysis = analyzeRecords([
    completedRecord(10),
    { ...completedRecord(10), schemaVersion: 8 },
    { ...completedRecord(10), schemaVersion: 5 },
    { ...completedRecord(10), schemaVersion: 10 },
    { ...completedRecord(10), state: "stalled", activeBashCommand: { text: "COMMAND-SENTINEL", totalBytes: 16, truncatedBytes: 0 } },
  ...
  assert.match(formatProgressGapAnalysis(analysis), /historical schema 3-8 2/);
  assert.doesNotMatch(JSON.stringify(analysis) + formatProgressGapAnalysis(analysis), /activeBashCommand|COMMAND-SENTINEL/);
});
...
test("scan tolerates malformed JSON and vanished files and keeps directory contents unchanged", async () => {
  ...
  await writeRecord("c-historical.json", { ...completedRecord(9), schemaVersion: 8 });
