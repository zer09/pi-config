...
| --- | --- | --- |
| `web_search` | Current, source-backed answers with inline citations | Gemini + Parallel grounding, then Gemini + Exa grounding after an eligible Parallel failure, then one direct Tavily `/search` call after an eligible Exa failure |
...
- `depth` omitted or `"standard"` uses Parallel mode `basic`, Exa `fast` with 5 results and 2000 highlight characters on fallback, and Tavily `search_depth: "basic"` with `max_results: 5` on the final fallback.
- `depth: "deep"` uses Parallel mode `advanced`, Exa `fast` with 10 results and 4000 highlight characters on fallback, and Tavily `search_depth: "advanced"` with `max_results: 10` on the final fallback.
- When a Gemini grounded answer is unavailable, `web_search` may return a degraded ordered source document (`## Search results` with titled URL entries) instead of provider-written prose; the calling model synthesizes the answer from that document and cites its URLs itself.
...
Default names (all optional to set; when the Google key is absent, `web_search` skips both Gemini stages and can use Tavily when configured):
...
TAVILY_API_KEY         Tavily key for the final direct web_search fallback
```

`PARALLEL_API_KEY` stays optional because Google Cloud Marketplace subscriptions can ground without a bring-your-own key. Firecrawl requests include `Authorization: Bearer` only when configured. A missing Google key is no longer terminal for `web_search`: both Gemini stages are recorded as skipped attempts and the Tavily fallback stays reachable; with Google, Exa, and Tavily all unconfigured the call makes zero HTTP requests and returns the unavailable outcome with three safe skips.
...
- Tavily: `POST https://api.tavily.com/search` with `Authorization: Bearer <key>` and exactly `query`, `search_depth`, `max_results`, `include_answer: false`, `include_raw_content: false`, and `include_usage: true`. No other keys (in particular `auto_parameters`) are ever sent.

## Tavily degraded output

When Tavily is the selected provider, `web_search` returns a degraded ordered source document instead of a grounded answer:
...
Results are provider-ordered; display indices are contiguous and zero-based over the delivered blocks, so a dropped URL (including one whose redacted form expands past the bound) leaves no index gap, and a result without a title uses the deterministic `Result N` label from its display index. Every provider string is redacted against the configured secrets, stripped of terminal control sequences, trimmed, and whitespace-collapsed before it is bounded: titles at 500 characters, snippets at 4000. A URL is never truncated: normalization drops any result whose URL exceeds 2000 characters. The document is assembled from whole result blocks and capped at 50 000 characters with a deterministic truncation marker that fits, so a URL can never be cut. The document and the safe details `resultCount` are built from one shared retained-result representation, so the count always equals the delivered result blocks. No citations, answer, or error text are synthesized; the calling model synthesizes and cites. At most 20 results are normalized per response, and a result survives only with a trimmed absolute http/https URL with a hostname plus non-empty title or content; score is kept only when finite.

## Fallback rules

Fallback is operational, never a second opinion. It starts for missing optional partner credentials when the provider cannot operate, transport failures, timeouts, HTTP 401/402/403/429/5xx, nested adapter errors, malformed responses, empty answers or content, no usable grounding sources, non-`STOP` grounding finishes, zero usable code-search results, and Firecrawl Scrape target pages whose own status (`metadata.statusCode`) is 4xx or 5xx even when the Firecrawl wrapper response is 2xx with non-empty error-page Markdown. Such target pages are recorded as `unusable_response`, their error-page Markdown is never returned or cached, and the URL moves to the Exa Contents fallback; absent, 2xx, and 3xx target statuses stay usable. Fallback never starts for caller cancellation, invalid local input, or a prompt safety block, and those terminal causes after either Gemini stage also prevent the Tavily fallback. A missing Google key skips both Gemini stages (recorded as safe skips) instead of failing the call; a missing Tavily key appends one skipped `tavily-search` attempt and returns the unavailable outcome. The intermittent nested Exa empty-query failure is retried exactly once; no other Exa failure retries automatically, Tavily is never retried, and the worst case is four serial calls (Parallel, Exa, one Exa retry, Tavily).
...
- `web_search` records (`schemaVersion: 3`, `tool` discriminator) add the authoritative bounded `selectedResult` discriminated by the exact provider (`provider: "gemini-parallel-grounding" | "gemini-exa-grounding" | "tavily-search"` with the bounded normalized payload, or null when nothing was selected), `selectedProvider` including `tavily-search`, and the chronological `attempts` list spanning both Gemini partners and the Tavily attempt. The legacy mirrored fields keep describing the final Gemini attempt: `provider` always keeps the final bounded Gemini attempt provider (`fallback?.provider ?? primary.provider`) and never stores `tavily-search`, a Tavily or none selection leaves the legacy `request`, `response`, and `normalized` mirrors absent or null and omits `googleResponseId` from storage entirely (safe details report it as null), while `primary`, `primaryAttempts`, and `fallback` keep the Gemini stage history even when Tavily was selected. Schema-2 and pre-schema legacy `web_search` records stay readable: a non-null legacy `normalized` mirror reads back as the Gemini selection its legacy provider fields describe.
- `web_code_search` records keep `schemaVersion: 2` with selected provider, chronological `attempts`, and `degraded`.
...
- Preflight records (`phase: "preflight"`) are persisted when a tool fails locally after execution begins: invalid input (`category: "invalid_input"`, limited to invalid parameters and URL normalization failures) or a configuration-load failure (`category: "config_load_failure"`). Missing credentials are never terminal preflight failures: they are recorded as skipped operational attempts (failure category `skipped_missing_credentials`) so the remaining providers stay reachable. Operational failures after validation (corrupt cache JSON, cache read failures other than a missing entry, cache write failures, and unexpected operational rejections) are rethrown unchanged: they get no preflight record and no diagnostic suffix. Preflight records store only the tool name, category, bounded sanitized error text, safe metadata available at the failure stage (never raw invalid parameter values), and an empty attempt list. When the configuration itself failed to load, the record uses the default cache directory, the default raw-response TTL, and the default credential env names for redaction.
...
- Tool details and the TUI summary include the `responseId` for `fetch_contents`, and all three tools expose safe summaries: selected provider (or none), provider list/attempt count, failure categories, and elapsed time. A Tavily selection additionally reports `degraded: true` and the delivered `resultCount` (exactly the result blocks in the document, after URL drops and whole-block total-cap truncation), rendered by the TUI as `provider=tavily-search degraded=true results=n` next to the attempt order, failures, elapsed time, and response ID, with no grounding counts, error strings, or credential env names. Raw provider errors never appear in model-visible output, tool details, or TUI summaries; failed fetch entries keep the generic `fetch failed` label.
- Failure categories classify by cause, checking the numeric HTTP status before any normalized body: `null` exactly when the same usability predicate that routes providers accepts the attempt (a parsed clean Gemini grounding answer, a Firecrawl success with artifacts, an Exa Code response with non-empty text and an absent or nonzero `resultsCount`, or a Tavily response with at least one surviving result). Any non-2xx status is `http_<status>` even when the body parsed, a 2xx body that did not parse is `unparsed`, grounding prompt blocks and non-`STOP` finishes are `blocked_<reason>` and `finish_<reason>`, a Tavily 2xx with a present but empty results array is `no_results`, and remaining causes are `no_results`, `provider_failure`, `empty_response`, `error`, `unusable`, or `skipped_missing_credentials`/`transport_error` when no HTTP status exists.
...
- Model-visible `fetch_contents` output and `web_code_search` output are each capped at 50 000 characters with a deterministic truncation marker; the code cap covers both the Exa Code response document and Firecrawl artifact/passage lists without changing provider selection or details. The Tavily degraded source document is capped at 50 000 characters assembled from whole result blocks with a fitting deterministic marker, titles at 500, snippets at 4000, and URLs never truncated (longer than 2000 characters drops the result).
...
- Stored schema versions: `web_search` records are `schemaVersion: 3` (the provider-discriminated `selectedResult`, `selectedProvider`, and the cross-provider `attempts` list are the schema-3 additions), while `web_code_search` and `fetch_contents` records stay at `2`. Every other change above is an in-field bound or an additive counter, previously written records remain readable, and readers tolerate records without the new fields (stored-body compatibility). No stored field changed its name or required shape.
...
Every stored request and response passes deep redaction covering `GOOGLE_CLOUD_API_KEY`, `PARALLEL_API_KEY`, `EXA_API_KEY`, `FIRECRAWL_API_KEY`, and `TAVILY_API_KEY`, including nested body copies, Tavily `Authorization` headers, normalized result fields, the selected result, and Tavily `request_id` values; preflight records use the configured (or default) credential env names for the same redaction. Complete secret values are replaced before any control stripping or truncation is applied, so a secret crossing a 500-, 4000-, or 20 000-character cutoff cannot survive as a partial fragment; storage-level deep redaction remains as defense in depth. Provider errors stay in private details and stored records; they never enter model-visible output, tool details, or the TUI summary.
