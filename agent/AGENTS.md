...
- When investigation is needed, run all configured solution roles concurrently; the delegates gather evidence and propose options, while the parent verifies the evidence, synthesizes conclusions, and remains sole author of the final plan or research deliverable. The configured solution roles derive from the routing snapshot in `agent/extensions/delegated-pi-loop/routing.json`.
- Normally every configured solution investigator must complete before synthesis. After one or more solution delegates fail operationally or end non-completed (blocked, cleanup_failed, routes_unavailable, and similar), the user may explicitly waive the named failed solution roles for that one current solution gate; once explicitly waived, continue synthesis using only the completed solution reports plus parent-verified repository evidence.
- At least one solution delegate must have completed; the user cannot waive the entire evidence set and synthesize from zero completed investigator reports. A solution waiver is one-shot and gate-scoped: record which solution roles were waived and that the solution gate proceeded under user waiver, never label a waived failure as completed or passed, and change no later solution gates, role schema, routing, or concurrency. Do not infer a waiver from a generic request to continue, commit, or skip retries; it requires precise user authorization for the named failed roles, such as "solution C may be waived for this gate". A waiver does not fabricate or dismiss evidence, resolve uncertainties, authorize implementation, replace parent evidence verification, skip the advisory solution oracle when otherwise required, or weaken implementation, review, verification, or remediation rules.
...
- For an accepted solution, run one implementation delegate; inspect its diff and evidence; then run all configured review roles concurrently.
- Normally every configured reviewer must complete. After one or more review delegates fail operationally or end non-completed (blocked, cleanup_failed, routes_unavailable, and similar), the user may explicitly waive the named failed reviewer roles for that one current gate; once explicitly waived, continue with the completed review reports instead of retrying or stopping solely because the waived reviewers failed.
- A reviewer waiver is one-shot and gate-scoped: record which reviewers were waived and that the gate completed under user waiver, never label a waived failure as a reviewer pass, and change no later gates, role schema, routing, or concurrency. Do not infer a waiver from a generic request to continue, commit, or skip retries; it requires precise user authorization for the named failed roles, such as "C may be waived for this gate". A waiver does not dismiss findings from completed reviewers or waive finding verification, remediation, oracle exclusivity, interruption handling, or other safety rules.
- Verify blocking findings with fresh finding-verification delegates: consolidate exact duplicates, run independent findings in batches of at most four, keep dependent findings sequential, wait for the whole batch, and send confirmed findings to one remediation delegate; repeat the full review gate until no blocking findings remain.
- Use `delegate_run.availableSkills` to make only task-relevant pre-approved skills discoverable to a delegate; selection does not force full skill loading, and the delegate decides which selected skills it actually needs.
