...
- When investigation is needed, run solution A/B/C/D/E/F concurrently; the delegates gather evidence and propose options, while the parent verifies the evidence, synthesizes conclusions, and remains sole author of the final plan or research deliverable.
- Normally all six solution investigators must complete before synthesis. After one or more solution delegates fail operationally or end non-completed (blocked, cleanup_failed, routes_unavailable, and similar), the user may explicitly waive the named failed solution roles for that one current solution gate; once explicitly waived, continue synthesis using only the completed solution reports plus parent-verified repository evidence.
...
- Verify blocking findings with fresh finding-verification delegates: consolidate exact duplicates, run independent findings in batches of at most four, keep dependent findings sequential, wait for the whole batch, and send confirmed findings to one remediation delegate; repeat the five-reviewer gate until no blocking findings remain.
