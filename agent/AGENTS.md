...
- Normally all four solution investigators must complete before synthesis. After one or more solution delegates fail operationally or end non-completed (blocked, cleanup_failed, routes_unavailable, and similar), the user may explicitly waive the named failed solution roles for that one current solution gate; once explicitly waived, continue synthesis using only the completed solution reports plus parent-verified repository evidence.
