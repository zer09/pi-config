...
- When a problem lacks an accepted solution contract, run two fresh read-only solution investigators concurrently before implementation. Give both the same neutral problem, verify their cited source evidence, and finalize the solution as orchestrator. Do not reuse investigators as implementers or reviewers.
