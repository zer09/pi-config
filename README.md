...
Delegation-specific policy is deliberately absent: the parent receives the delegated workflow exactly once through the active `delegate_run` guidelines in `agent/extensions/delegated-pi-loop/instructions.ts`. The general `OVERRIDE:` mechanism in `AGENTS.md` is user-controlled instruction precedence, not a workflow step or permission that an agent may request. Delegated children load `AGENTS.md` without receiving parent delegation policy.
