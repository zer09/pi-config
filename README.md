...
Delegation-specific policy is deliberately absent: the parent receives the delegated workflow exactly once through the active `delegate_run` guidelines in `agent/extensions/delegated-pi-loop/instructions.ts`, while the general `OVERRIDE:` mechanism in `AGENTS.md` applies uniformly to every agent workflow. Delegated children load `AGENTS.md` without receiving parent delegation policy.
...
| `delegated-pi-loop/` | Native TypeScript `delegate_run` tool with persistent Pi RPC children, config-driven version-2 routing (role families derive from ordered `assignments` arrays in `routing.json`, with one normalized registry for validation, classification, contracts, and generated schemas/guidance), strict delegated child resource isolation, a read-only `delegate_model_catalog` lookup for exceptional one-run substitutions, programmatic 45-minute work and 10-minute idle deadlines, sequential operational fallback, bounded cleanup, schema 5 safe diagnostics, one same-session report recovery, and role isolation. Model-visible delegation text is centralized in `instructions.ts`: flat parent guidelines name their tool, children receive compact role prompts without parent workflow or mental time tracking, and the extension references the general `OVERRIDE:` mechanism for failed-gate authorization. Direct Claude CLI delegation is removed; Claude-named models remain available through normal Pi providers. |
